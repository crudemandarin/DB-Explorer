        ...
        return tableFields.reduce(
            (previous, tableField) => {
                if (Utils.getProtectedFields().includes(tableField.name)) {
                    if (tableField.isPrimaryKey) return [...previous, { name: tableField.name, value: Utils.getTableID() }];
                    if (['CreatedAt', 'UpdatedAt'].includes(tableField.name)) return [...previous, { name: tableField.name, value: Date.now() }];
                }
